...
def print_death_age(people_dict):
    ...
    Return: nothing
    """
    for key, data in people_dict.items():
        print(f"{data[0]} died at {data[3] - data[2]} years.")
...
def count_genders(people_dict):
    ...
    """
    m, f = 0, 0
    for data in people_dict.values():
        if data[1] == "M":
            m += 1
        else:
            f += 1
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for key, data in marriages_dict.items():
        man = people_dict[data[0]]
        woman = people_dict[data[1]]
        print(f"In {data[2]}, at age of {data[2] - man[2]}, {man[0]} got married to {woman[0]} who was {data[2] - woman[2]}")
